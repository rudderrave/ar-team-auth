...
namespace arteam\auth\controllers;

use arteam\auth\assets\AvatarAsset;
use arteam\auth\AuthModule;
use arteam\auth\helpers\AvatarHelper;
use arteam\auth\models\Auth;
use arteam\auth\models\forms\ConfirmEmailForm;
use arteam\auth\models\forms\LoginForm;
use arteam\auth\models\forms\ResetPasswordForm;
use arteam\auth\models\forms\SetEmailForm;
use arteam\auth\models\forms\SetPasswordForm;
use arteam\auth\models\forms\SetUsernameForm;
use arteam\auth\models\forms\SignupForm;
use arteam\auth\models\forms\UpdatePasswordForm;
use arteam\components\AuthEvent;
use arteam\controllers\BaseController;
...
class DefaultController extends BaseController
{
    ...
    {
        return [
            'captcha' => Yii::$app->arteam->captchaAction,
            'oauth' => [
                'class' => 'arteam\auth\AuthAction',
            ...
            if ($auth) { // login
                ...
                if (isset($attributes['email']) && $attributes['email'] && User::find()->where(['email' => $attributes['email']])->exists()) {
                    Yii::$app->getSession()->setFlash('error', [
                        Yii::t('arteam/auth', "User with the same email as in {client} account already exists but isn't linked to it. Login using email first to link it.", ['client' => $client->getTitle()]),
                    ]);
    ...
    protected function createUser($attributes)
    {
        ...
        if ($user->save()) {
            ...
            if ($auth->save()) {
                ...
                Yii::$app->session->setFlash('error', 'Error 901: ' . Yii::t('arteam/auth', "Authentication error occurred."));
            ...
            Yii::$app->session->setFlash('error', 'Error 902: ' . Yii::t('arteam/auth', "Authentication error occurred."));
            return Yii::$app->response->redirect(Url::to(['/auth/default/login']));
        }
    ...
    public function actionConfirmRegistrationEmail($token)
    {
        if (Yii::$app->arteam->emailConfirmationRequired) {
            ...
            throw new NotFoundHttpException(Yii::t('arteam/auth', 'Token not found. It may be expired'));
        }
    }
        ...
        if ($model->load(Yii::$app->request->post()) AND $model->validate()) {
            if ($this->triggerModuleEvent(AuthEvent::BEFORE_PASSWORD_RECOVERY_REQUEST, ['model' => $model])) {
                if ($model->sendEmail(false)) {
                    ...
                    Yii::$app->session->setFlash('error', Yii::t('arteam/auth', "Unable to send message for email provided"));
                }
            }
        }
    ...
    public function actionResetPasswordRequest($token)
    {
        ...
        if (!$user) {
            throw new NotFoundHttpException(Yii::t('arteam/auth', 'Token not found. It may be expired. Try reset password once more'));
        }
    ...
    protected function triggerModuleEvent($eventName, $data = [])
    {
        ...
        Yii::$app->arteam->trigger($eventName, $event);
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->validate()) {
                try {
                    ...
                    return Yii::t('arteam', 'An unknown error occurred.');
                }
            } else {
