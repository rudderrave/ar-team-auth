...
class LoginForm extends Model
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'rememberMe' => Yii::t('arteam/auth', 'Remember me'),
        ];
    }
    ...
    public function validatePassword()
    {
        if (!Yii::$app->arteam->checkAttempts()) {
            $this->addError('password', Yii::t('arteam/auth', 'Too many attempts'));
            return false;
        }
        ...
        if (!$this->hasErrors()) {
            ...
            if (!$user || !$user->validatePassword($this->password)) {
                $this->addError('password', Yii::t('arteam/auth', 'Incorrect username or password'));
            }
        }
    }
    ...
    public function validateIP()
    {
        ...
        if ($user AND $user->bind_to_ip) {
            ...
            if (!in_array(YeeHelper::getRealIp(), $ips)) {
                $this->addError('password', Yii::t('arteam/auth', "You could not login from this IP"));
            }
        }
    }
