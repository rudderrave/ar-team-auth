...
class UpdatePasswordForm extends Model
{
    ...
    {
        return [
            'current_password' => Yii::t('arteam/auth', 'Current password'),
    ...
    public function validateCurrentPassword()
    {
        if (!Yii::$app->arteam->checkAttempts()) {
            $this->addError('current_password', Yii::t('arteam/auth', 'Too many attempts'));
            return false;
        }

        if (!Yii::$app->security->validatePassword($this->current_password, $this->user->password_hash)) {
            $this->addError('current_password', Yii::t('arteam/auth', "Wrong password"));
        }
    }
